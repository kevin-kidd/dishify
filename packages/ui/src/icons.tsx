...
import type { LucideIcon as BaseLucideIcon } from "lucide-react-native";
import * as BaseLucideIcons from "lucide-react-native";

export function LucideIcon(icon: BaseLucideIcon) {
  cssInterop(icon, {
    className: {
      target: "style",
      nativeStyleToProp: {
...
// Type guard to check if an icon is a BaseLucideIcon
function isBaseLucideIcon(icon: unknown): icon is BaseLucideIcon {
  return typeof icon === "function" && "displayName" in icon;
}

// Export wrapped Lucide icons
function WrappedLucideIcons() {
  return Object.entries(BaseLucideIcons).reduce(
    (acc, [name, icon]) => {
      if (isBaseLucideIcon(icon)) {
        acc[name] = LucideIcon(icon);
      }
      return acc;
    },
    {} as Record<string, ReturnType<typeof LucideIcon>>
  );
}
...
const CustomIcons = {
  LeftArrow: ({ className, fill = "#000000", ...rest }: IconProps) => (
  ...
  GoogleIcon: ({ className, fill = "#000000", ...rest }: IconProps) => (
  ...
  LinkedInIcon: ({ className, fill = "#000000", ...rest }: IconProps) => (
